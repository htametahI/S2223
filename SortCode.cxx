...
double tigs3T[2] = {0, 800}; // TIGRESS - S3 Timing. Change !!!
...
//  TChain *AnalysisTree = (TChain *)analysisfile->Get("AnalysisTree");
  ...
  //double_t particle_betaDoppler =  0.08647; //particle_beta for Doppler correction for 22Ne at 3.5 MeV/u
  //double_t particle_betaDoppler =  0.080056; //...and scaled for 3.0 MeV/u
  ...
  {
    //std::cout << "\nEntry number " << jentry;fflush(stdout);
      ...
      for (int t = 0; t < tigress->GetAddbackMultiplicity(); t++){
        ...
        if (!suppAdd && add_hit->GetEnergy() > 15){
          addE->Fill(add_hit->GetEnergy());
					addDoppRaw->Fill(add_hit->GetDoppler(particle_betaDoppler));
          addE_ANum->Fill(add_hit->GetArrayNumber(), add_hit->GetEnergy());
          addDoppRaw_ANum->Fill(add_hit->GetArrayNumber(), add_hit->GetDoppler(particle_betaDoppler));
    ...
    //std::cout << "\nMade it to start of S3 loop!";fflush(stdout);
    if (s3){ // S3 singles
	...
	s3_E_ringNum->Fill(ring_hit->GetRing(),ring_hit->GetEnergy());
      ...
      if (std::abs(ringEnergy - sectorEnergy) < 100.0){ //check if difference in ring and sector energies is less than 100 keV for this event
        s3EnergyDiff = true;
	//continue;
      }
...
//std::cout << "\nMade it to start of EMMA loop!";fflush(stdout);
	...
	//wooh was here
      for (int e = 0; e < emma->GetMultiplicity(); e++)
      {
		...
		for (int tigIndex = 0; tigIndex < tigress->GetAddbackMultiplicity(); tigIndex++){
			...
			if (800. > timeCut || 1000. < timeCut) continue;
		 ...
		 for (int sectorIndex = 0; sectorIndex < s3->GetSectorMultiplicity(); sectorIndex++){
			sector_hit = s3->GetSectorHit(sectorIndex);		 	
			
			//if (1 != s3->GetRingMultiplicity() || 1 != s3->GetSectorMultiplicity()) continue;	
					


			if (0 == ring_hit->GetRing()){
				s3_ring0_mg_pid_gated->Fill(ring_hit->GetEnergy());
			}
			if (1 == ring_hit->GetRing()){
				s3_ring1_mg_pid_gated->Fill(ring_hit->GetEnergy());
			}
			if (20 == ring_hit->GetRing()){
				s3_ring20_mg_pid_gated->Fill(ring_hit->GetEnergy());
			}
			if (21 == ring_hit->GetRing()){
				s3_ring21_mg_pid_gated->Fill(ring_hit->GetEnergy());
			}
			if (0 == sector_hit->GetSector()){
				s3_sector0_mg_pid_gated->Fill(sector_hit->GetEnergy());
			}
			if (1 == sector_hit->GetSector()){
				s3_sector1_mg_pid_gated->Fill(sector_hit->GetEnergy());
			}
			if (2 == sector_hit->GetSector()){
				s3_sector2_mg_pid_gated->Fill(sector_hit->GetEnergy());
			}
			if (3 == sector_hit->GetSector()){
				s3_sector3_mg_pid_gated->Fill(sector_hit->GetEnergy());
			}
	...
	//s3 rings in coincidence with emma PID
		...
		//std::cout << "\nTig loop in emma loop";fflush(stdout);				
            if(tigress){
              ...
              //std::cout << "\nEnergy " << add_hit->GetEnergy();fflush(stdout);
                ...
                double timeCut = add_hit->GetTime() - si_hit->GetTime();
                // std::cout << "\nTimecut is " << timeCut;fflush(stdout);
                // ==== Delay Time Window for 26Mg gamma spectrum ==== //
                //if((timeCut>=600 && timeCut<=800) || (timeCut>=1000 && timeCut<=1200)){
		  //if(Mg26_cut->IsInside(si_hit->GetEnergy(), tempIC)){
		   // doppE_26Mg_pidg_delayT->Fill(add_hit->GetDoppler(particle_betaDoppler));
		  //}
		//}
                // =================================================== //
		...
		if (800. > timeCut || 1000. < timeCut) continue;
		 ...
		 // std::cout << "\nS3 multiplicity is " << s3->GetPixelMultiplicity();fflush(stdout);
		  for (int s3index = 0; s3index < s3->GetPixelMultiplicity(); s3index++){ //requires S3 hit now
          ...
          //std::cout << "\ntig multiplicity is " << tigress->GetAddbackMultiplicity();fflush(stdout);
                ...
                suppAdd = add_hit->BGOFired();
                if (!suppAdd && add_hit->GetEnergy() > 15){
                 // printf("energy: %f\n",add_hit->GetDoppler(particle_betaDoppler));fflush(stdout);
                  //if(add_hit->GetDoppler(particle_betaDoppler) > 1254){
                  //  if(add_hit->GetDoppler(particle_betaDoppler) < 1294){
                   //   icSumVicPlusSi1274Gate->Fill(tempIC+si_hit->GetEnergy(),tempIC);
                   // }
                  //}std::cout << "\nMade it here!";fflush(stdout);
                }
              }
            }//std::cout << "\nMake it here?";fflush(stdout);
          }
        }//before here
...
//std::cout << "\nMade it to start of EMMA and S3 and TIGRESS loop!";fflush(stdout);
          if (s3 && tigress){ // EMMA + S3 + TIGRESS coincidences
            for (int i = 0; i < s3->GetPixelMultiplicity(); i++){
              s3hit = s3->GetPixelHit(i);
              for (int t = 0; t < tigress->GetAddbackMultiplicity(); t++){
                add_hit = tigress->GetAddbackHit(t);
                ...
                if (!suppAdd && add_hit->GetEnergy() > 15){
                  if (gate1D((add_hit->GetTime() - s3hit->GetTime()), tigs3T[0], tigs3T[1])){
                    ...
                    particle_beta = ne21->AnalysisBeta(rekin * 1e-3, 3); //get laboratory particle_beta of the recoil
                        ...
                        if (tofGate((s3hit->GetTime() - em_hit->GetTime()))){ // TOF gate and
                          ...
                          /*if (CutG_loaded)
                          {
                            ...
                            if (siicCut && s3EnergyDiff)
                            {
                              ...
                              addDopp_PIDG_bg->Fill(add_hit->GetDoppler(particle_beta, &recoil_vec));
                              addDopp_ANum_PIDG_bg->Fill(add_hit->GetArrayNumber(), add_hit->GetEnergy());
                              addDopp_exc_PIDG_bg->Fill(add_hit->GetDoppler(particle_beta, &recoil_vec), exc);
                            }
                          }
                          else
                          {
    ...
    if (jentry == 16875000){
	std::cout << "\nMade it to entry 16890712";fflush(stdout);
    }
  } // analysis tree
